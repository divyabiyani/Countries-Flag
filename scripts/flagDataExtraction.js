...
var dataInsert = function(json) {
  ...
  for(var i=0;i<length;i++) {
    dataIns += '<li><div class="box effect1"><p id="name"><strong>' + json[i].name.toUpperCase() +'</strong></p><div class="image-effect"><img src=' + json[i].flag + '></div><p id="latlong">' + json[i].latlng[0] + ', ' + json[i].latlng[1] + '</p><div class="overlay"><summary><h2>' + json[i].name + '</h2><h3>Capital: ' + json[i].capital +  '</h3><br/><h3>Latitude: ' + json[i].latlng[0] + '</h3><br/><h3>Longitute: ' + json[i].latlng[1] +'</summary></div></div></li>';
  }
  dataIns += '</ul>';
